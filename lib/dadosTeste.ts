import type { TipoVinho, Confianca } from "@prisma/client";
import { prisma } from "./prisma";
import { regenerarPairingsSemIA } from "./orquestracao";

/**
 * Menu e carta de teste, com atributos escritos à mão em vez de gerados pela
 * IA — corre sem gastar crédito nenhum, e o pairing sai a seguir pelo motor
 * de regras (lib/pairingRegras.ts). Partilhado entre o script de CLI
 * (prisma/seedMenuCompleto.ts) e a rota de backoffice que faz o mesmo sem
 * precisar de acesso a terminal.
 */
...
export const PRATOS_TESTE: Prato[] = [
  // --- Entradas ---
  {
...
export const VINHOS_TESTE: Vinho[] = [
  {
    nome: "Vinho Verde",
...
export type ResultadoSeedTeste = { pratosCriados: number; vinhosCriados: number };

export async function semearMenuTeste(restauranteId: string): Promise<ResultadoSeedTeste> {
  const agora = new Date();
  let pratosCriados = 0;
  let vinhosCriados = 0;

  for (const p of PRATOS_TESTE) {
    const existe = await prisma.prato.findFirst({
      where: { restauranteId, nome: p.nome },
    });
    if (existe) continue;
  ...
  for (const v of VINHOS_TESTE) {
    ...
    const jaNaCarta = await prisma.cartaVinho.findFirst({
      where: { restauranteId, vinhoCatalogoId: vinhoCatalogo.id },
    });
  ...
  await regenerarPairingsSemIA(restauranteId);
  return { pratosCriados, vinhosCriados };
}
